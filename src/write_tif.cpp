      ...
      const string& outTifPrefix )
{
   ...
   if ( write_tif_grayscale<uint16_t>(
            ...
            false )  == EXIT_FAILURE)
   {
      cerr << "Error : write_tif_grayscale16_append() failed" << endl;
      return EXIT_FAILURE;
   }
   return EXIT_SUCCESS;
...
//-----------------------------------------------------------------------
int TEM_NS::writeImage::write_tif_grayscale16_append(
      const double* const outDataRaw, 
      const double& min, const double& max,// bounds of scale for input data
      const size_t& height, const size_t& width,// image dimensions [pixels]
      const size_t& resolutionUnit,
      ...
      const string& outTifPrefix )
{
   ...
   // write the image
   /////////////////////////////////////////////////////////////////////////
   if ( write_tif_grayscale_append<uint16_t>(
            outDataRaw,
            min, max,
            height, width, 
            resolutionUnit,
      ...
      const string& outTifPrefix )
{
   ...
   return EXIT_SUCCESS;
} // write_tif_grayscale16_logscale
      ...
      )
{
   ...
   if ( mynode == rootnode )
   {
      ...
      imageWriter.write_tif_grayscale16(
            ...
            resolutionUnit,
            xResolution, yResolution,
            outFileName_prefix + "_diffraction_stack_scalefactor_" 
...
//-----------------------------------------------------------------------
int TEM_NS::output_diffraction_with_renormalization_append(
      const fftw_complex* const psi,
      const double& scale_factor,
      const ptrdiff_t& local_alloc_size_fftw,
      const ptrdiff_t& Nx_local,
      ...
      )
{
   ...
   if ( mynode == rootnode )
   {
      ...
      imageWriter.write_tif_grayscale16_append(
            psi_mag_joined,
            min_psi_mag_joined,
            max_psi_mag_joined,
            Nx, Ny,
